"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProductsRepository = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
let ProductsRepository = class ProductsRepository {
    productModel;
    productItemModel;
    constructor(productModel, productItemModel) {
        this.productModel = productModel;
        this.productItemModel = productItemModel;
    }
    async findAll(filter = {}, sort = {}, skip = 0, limit = 10) {
        return await this.productModel
            .find(filter)
            .populate('category')
            .sort(sort)
            .skip(skip)
            .limit(limit)
            .lean();
    }
    async count(filter = {}) {
        return await this.productModel.countDocuments(filter);
    }
    async findById(id) {
        return await this.productModel.findById(id).lean();
    }
    async findItemsByProductId(productId) {
        return await this.productItemModel.find({ product: productId }).lean();
    }
    async updateStock(productId, quantity) {
        return await this.productModel.findByIdAndUpdate(productId, { $inc: { totalStock: quantity } }, { new: true });
    }
};
exports.ProductsRepository = ProductsRepository;
exports.ProductsRepository = ProductsRepository = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)('Product')),
    __param(1, (0, mongoose_1.InjectModel)('ProductItem')),
    __metadata("design:paramtypes", [mongoose_2.Model,
        mongoose_2.Model])
], ProductsRepository);
//# sourceMappingURL=products.repository.js.map